...
use clap_clap::{
    ...
    ffi::{
        ...
        clap_audio_port_info, clap_gui_resize_hints, clap_host, clap_host_params, clap_host_state,
...
impl AudioProcessor {
    fn new(shared: &SharedState, max_frames: u32) -> Self {
...
unsafe extern "C-unwind" fn plugin_activate(
    ...
    let next = Box::into_raw(Box::new(AudioProcessor::new(&instance.shared, max_frames)));
...
unsafe extern "C-unwind" fn ext_audio_ports_get(
    ...
    let name = if index == 0 { "in_l" } else { "in_r" };
...
unsafe extern "C-unwind" fn ext_gui_is_api_supported(
    ...
    crate::formant::gui::is_api_supported(api, is_floating)
}
...
unsafe extern "C-unwind" fn ext_gui_get_preferred_api(
    ...
    let preferred = crate::formant::gui::preferred_api();
    unsafe {
...
unsafe extern "C-unwind" fn ext_gui_get_size(
    ...
    unsafe {
        *width = crate::formant::gui::EDITOR_WIDTH;
        *height = crate::formant::gui::EDITOR_HEIGHT;
    }
    true
}
...
unsafe extern "C-unwind" fn ext_gui_set_parent(
    ...
    let parent = if api == CLAP_WINDOW_API_X11 {
        #[cfg(unix)]
        {
            crate::formant::gui::ParentWindowHandle::X11(unsafe { window.clap_window__.x11 })
        }
        #[cfg(not(unix))]
        {
            return false;
        ...
        {
            crate::formant::gui::ParentWindowHandle::Win32(unsafe { window.clap_window__.win32 })
        }
